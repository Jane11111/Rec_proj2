...
class MLP:
    ...
    def fit(self,U,I,Y):
        ...
        pred_y=tf.layers.dense(inputs=last_vec,units=1,activation=tf.nn.sigmoid,use_bias=True)
        ...
        reg_loss=tf.reduce_mean(tf.reduce_mean(tf.nn.l2_loss(Wu_mlp))+tf.reduce_mean(tf.nn.l2_loss(Wi_mlp)))
    ...
    def predict(self,U,I):
        ...
        with tf.Session() as sess:
            saver = tf.train.import_meta_graph("model/mlp.meta")
            saver.restore(sess,'model/mlp')
